...
class acp_shareon
{
	...
	function main($id, $mode)
	{
		...
		if ($submit)
		{
			...
			set_config('so_vk', request_var('so_vk', 0));
			set_config('so_mail', request_var('so_mail', 0));
			set_config('so_odnokl', request_var('so_odnokl', 0));
			set_config('so_blogspot', request_var('so_blogspot', 0));
			set_config('so_yaru', request_var('so_yaru', 0));
			set_config('so_livej', request_var('so_livej', 0));
			set_config('so_gbuzz', request_var('so_gbuzz', 0));
		...
		$template->assign_vars(array(
			...
			'SO_VK'			=> $config['so_vk'],
			'SO_MAIL'		=> $config['so_mail'],
			'SO_ODNOKL'		=> $config['so_odnokl'],
			'SO_BLOGSPOT'	=> $config['so_blogspot'],
			'SO_YARU'		=> $config['so_yaru'],
			'SO_LIVEJ'		=> $config['so_livej'],
			'SO_GBUZZ'		=> $config['so_gbuzz'],
